function X = CKM2(A, f, poles, debug, normfA, isherm, onlyd)
...
if ~exist('poles', 'var') 
	...
	if onlyd == 0
		X = blkdiag(CKM2(DA1, f, poles, debug, normfA, isherm, onlyd), CKM2(DA2, f, poles, debug, normfA, isherm, onlyd)); % function of the diagonal blocks
	elseif onlyd == 1
		X = [CKM2(DA1, f, poles, debug, normfA, isherm, onlyd); CKM2(DA2, f, poles, debug, normfA, isherm, onlyd)];
	elseif onlyd == 2
		X = CKM2(DA1, f, poles, debug, normfA, isherm, onlyd) + CKM2(DA2, f, poles, debug, normfA, isherm, onlyd);
	else
		error('HSS_FUN_DAC:: unsupported value of onlyd')
...
%-----------------------END DEBUG----------------------------------------------------------------------------------------
		...
		for j = 1:maxit
			...
			elseif length(poles) == 2 && (prod(poles == [0 inf]) || prod(poles == [inf 0])) % extended Krylov method
				...
				[V1, KK1, HH1, ~] = rk_krylov(DA, VV1, K1, H1, inf, params1); % artificial step at infinity to get the projected matrices 
				[V2, KK2, HH2, ~] = rk_krylov(DA, VV2, K2, H2, inf, params2);
			end
